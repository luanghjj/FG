            ...
            {filteredDetails.map(({ question: q, userSet, correctSet, isCorrect }, idx) => (
              ...
              >
                ...
                {(() => {
                  ...
                  if (!exp) return null
                  return (
                    <div className="mt-3 p-3.5 rounded-xl bg-amber-50/90 border border-amber-200 text-xs text-gray-800 space-y-2.5">
                      <div className="flex items-center justify-between flex-wrap gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {exp.isCrucial && (
                            <span className="text-[10px] font-extrabold text-red-700 bg-red-100 px-1.5 py-0.5 rounded border border-red-200 flex items-center gap-0.5">
                      ...
                      <p className="text-gray-800 leading-relaxed font-medium">{exp.content}</p>

                      {/* Formula Calculation */}
                      {exp.formulaCalculation && (
                        <div className="p-2.5 bg-white/90 border border-amber-300 rounded-lg font-mono text-[11px] text-amber-950 space-y-0.5">
                          <p className="font-bold font-sans text-amber-900 text-[10px]">Công thức tính toán từng bước:</p>
                          <p className="whitespace-pre-line leading-relaxed">{exp.formulaCalculation}</p>
                        </div>
                      )}

                      {/* Correct reasons */}
                      {exp.correctBreakdown && exp.correctBreakdown.length > 0 && (
                        <div className="space-y-1">
                          <p className="text-[11px] font-bold text-green-900">Vì sao phương án đúng được chọn?</p>
                          {exp.correctBreakdown.map((item, bIdx) => (
                            <div key={bIdx} className="bg-white/80 p-1.5 rounded-lg border border-green-200 text-[11px] text-gray-700 flex items-start gap-1">
                              <Check className="w-3 h-3 text-green-600 shrink-0 mt-0.5" />
                              <div>
                                <span className="font-semibold text-gray-900">{item.de}</span>
                                {item.reason && <p className="text-green-800 font-medium">{item.reason}</p>}
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {/* Trap warnings */}
                      {exp.trapBreakdown && exp.trapBreakdown.length > 0 && (
                        <div className="space-y-1">
                          <p className="text-[11px] font-bold text-red-900">Bẫy cần tránh ở phương án sai:</p>
                          {exp.trapBreakdown.map((item, bIdx) => (
                            <div key={bIdx} className="bg-white/80 p-1.5 rounded-lg border border-red-200 text-[11px] text-gray-700 flex items-start gap-1">
                              <X className="w-3 h-3 text-red-500 shrink-0 mt-0.5" />
                              <div>
                                <span className="line-through text-gray-600">{item.de}</span>
                                {item.trap && <p className="text-red-700">{item.trap}</p>}
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {exp.keyTakeaway && (
                        <div className="text-[11px] text-amber-950 font-bold pt-1.5 border-t border-amber-200/80 flex items-start gap-1">
                          <Sparkles className="w-3 h-3 text-amber-600 shrink-0 mt-0.5" />
                          <span>Mẹo cốt lõi: {exp.keyTakeaway}</span>
                        </div>
                      )}
                    </div>
                  )
